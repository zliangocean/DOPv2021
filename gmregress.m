function [b,bintr,bintjm,p] = gmregress(x,y,alpha)
% add p value calculation on 03/16/2021,modified by Zhou Liang zl18c@my.fsu.edu
...
%      A MATLAB file. [WWW document]. URL https://urldefense.com/v3/__http://www.mathworks.com/__;!!Epnw_ITfSMW4!6iuEfH3uGYPp8QXfwx70_ndv8tSuBwqzSKR1vIvS5DYa6fP4YNKPzwT2GrRBuwwTaQ$ 
...
b = [u v r n];
% calculate SE of the slope
% the equation is used from https://stattrek.com/regression/slope-test.aspx
SE = sqrt(sum((y-(v.*x+u)).^2)/(n-2))/sqrt(sum((x-mean(x)).^2));
tvalue = v / SE;
% deal with negative tvalue
if tvalue>0
    tvalue = tvalue;
else
    tvalue = -tvalue;
